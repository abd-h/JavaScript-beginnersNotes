...
because they use special characters to make more complex, 
flexible matches.
...
JavaScript has multiple ways to use regexes. One way to test 
a regex is using the .test() method. 
...
In the last challenge, you searched for the word Hello using 
...
/*A future challenge will show how to match those other forms 
as well.
...
This is powerful to search single strings, but it's limited to 
only one pattern. You can search for multiple patterns using 
...
Complete the regex petRegex to match the pets dog, cat, bird, 
or fish.
...
There are other flags but here you'll focus on the flag that 
ignores case - the i flag. You can use it by appending it to the 
regex. An example of using this flag is /ignorecase/i. 
This regex can match the strings ignorecase, igNoreCase, 
and IgnoreCase.

Write a regex fccRegex to match freeCodeCamp, no matter its case. 
Your regex should not match any abbreviations or variations 
with spaces.*/
